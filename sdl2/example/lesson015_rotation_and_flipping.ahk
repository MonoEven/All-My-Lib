...
; Version: v1.0.0
; Time: 2022.09.15
; SDL 2's hardware accelerated texture rendering also gives us the ability to to do fast image flipping and rotation. In this tutorial we'll be using this to make an arrow texture spin and flip.
...
loadMedia()
{
	...
	; Load arrow
	if (!gArrowTexture.loadFromFile("BMP\arrow.png"))
	{
		MsgBox SDL2.Log("Failed to load arrow texture!`n")
		success := false
	}
            ...
            while (SDL2.PollEvent(&e) != 0)
            {
                ...
                else if( e.type == SDL.SDL_KEYDOWN )
                {
                    switch(e.key.keysym.sym)
                    {
                        ...
                        case SDL.SDLK_q:
                            flipType := SDL.SDL_FLIP_HORIZONTAL

                        case SDL.SDLK_w:
                            flipType := SDL.SDL_FLIP_NONE

                        case SDL.SDLK_e:
                            flipType := SDL.SDL_FLIP_VERTICAL
                    }
                }
            }
