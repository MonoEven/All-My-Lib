class ahkbase
{
    static nullptr := ahkbase.Null()
    
    class Array
    {
        mItem := ahkbase.nullptr
        mLength := 0
        mCapacity := 0
        ...
        __new(arr)
        {
            if arr is Array
            {
                prop := []
                prop.length := 3
                _mcode := "2,x64:SIsCSIsISItCCEyLAUiLCEiLEUiLQiDHQAgBAAAASItKIEmLQCBIiQFIi0Igx0AYAQAAAEiLQiBBi0goSIlIEEiLQiDHQCgBAAAASItCIEGLSCxIiUggww=="
                fn := ahkbase.native.func(_mcode, 2, 2)
                fn(arr, prop)
                this.mItem := prop[1]
                this.mLength := prop[2]
                this.mCapacity := prop[3]
            }
        }
    }
    ...
    class Map
    {
        ...
        __new(mmap)
        {
            if mmap is Map
            {
                prop := []
                prop.length := 6
                _mcode := "2,x64:SIsCSIsISItCCEyLAUiLCEiLEUiLQiDHQAgBAAAASItKIEmLQCBIiQFIi0Igx0AYAQAAAEiLQiBBi0goSIlIEEiLQiDHQCgBAAAASItCIEGLSCxIiUggSItCIMdAOAEAAABIi0IgSMdAMAAAAABIi0Igx0BIAQAAAEiLQiBBi0gwSIlIQEiLQiDHQFgBAAAASItCIEGLSDRIiUhQww=="
                fn := ahkbase.native.func(_mcode, 2, 2)
                fn(mmap, prop)
                this.mItem := prop[1]
                this.mCount := prop[2]
                this.mCapacity := prop[3]
                this.mKeyOffsetInt := prop[4]
                this.mKeyOffsetObject := prop[5]
                this.mKeyOffsetString := prop[6]
            }
        }
    }
    ...
    class ResultType
    {
        ...
        static OK := 1
        static WARN := this.OK
        static CRITICAL_ERROR := 2
        static CONDITION_TRUE := 3
        static CONDITION_FALSE := 4
        static LOOP_BREAK := 5
        static LOOP_CONTINUE := 6
        static EARLY_RETURN := 7
        static EARLY_EXIT := 8
        static FAIL_OR_OK := 9
    }
    ...
    class SymbolType
    {
        ...
        static PURE_FLOAT := 2
        static SYM_STRING := this.PURE_NOT_NUMERIC
        static SYM_INTEGER := this.PURE_INTEGER
        static SYM_FLOAT := this.PURE_FLOAT
        static SYM_MISSING := 3
        static SYM_VAR := 4
        static SYM_OBJECT := 5
        static SYM_DYNAMIC := 6
    }
    ...
    class Variant
    {
        __new(prop)
        {
            ...
            switch this.symbol
            {
                case ahkbase.SymbolType.SYM_INTEGER:
                    this.n_int64 := prop[1]
                case ahkbase.SymbolType.SYM_FLOAT:
                    this.n_double := prop[1]
                case ahkbase.SymbolType.SYM_OBJECT:
                    this.object := prop[1]
                case ahkbase.SymbolType.SYM_STRING:
                    this.string := prop[1]
                case ahkbase.SymbolType.SYM_DYNAMIC:
                    this.prop := prop[1]
                default:
                {
                    this.symbol := ahkbase.SymbolType.SYM_MISSING
                    this.prop := ahkbase.nullptr
                }
            }
            this.key_c := "chr(" prop[3] ")<--may be wrong"
        }
        
        static getItems(varPtr, length := ahkbase.nullptr)
        {
            if length == ahkbase.nullptr
            {
                if !(varPtr is ahkBase.Array)
                    throw error("input type error.")
                else
                    length := varPtr.mLength
            }
            if varPtr is ahkBase.Array
                varPtr := varPtr.mItem
            prop := []
            ...
            _mcode := "2,x64:"
            fn := ahkbase.native.func(_mcode, 3, 3)
            fn(varPtr, items, length)
            loop items.length
                items[a_index] := ahkbase.Variant(items[a_index])
            return items
        }
    }
    ...
    ***********************************************************************
    ...
    class native extends func
    {
        ...
        static func(bif, minparams := 0, paramcount := 0, outputvars := 0, fid := 0)
        {
            ...
            numput('uint', 1, 'uint', 0, 'ptr', objptraddref(ahkbase.native.prototype), sbif, A_PtrSize)
            ; init func infos
        ...
        /**
         * Generate a method with native code, is same with `ahkbase.native.func`
        ...
        static method(base, bim, mit, minparams := 0, paramcount := 0, mid := 0)
        {
            ...
            numput('uint', 1, 'uint', 0, 'ptr', objptraddref(ahkbase.native.prototype), sbim, A_PtrSize)
        ...
        static loadmodule(path, load_symbols := 0, loader := 0, provider := 0)
        {
            ...
            if !p := dllcall(module_load_addr, 'ptr', objptr(loader || default_loader), 'ptr', objptr(provider || ahkbase.native), 'cdecl ptr')
        ...
        static mdfunc(fn, sig, prototype := 0)
        {
            ...
            ; init func refcount and base obj
            numput('uint', 1, 'uint', 0, 'ptr', objptraddref(ahkbase.native.prototype), smdf, A_PtrSize)
            ; init func infos
