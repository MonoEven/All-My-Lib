...
class LTexture
{
    ...
    render(x, y)
    {
        ...
        SDL2.RenderCopy(gRenderer, this.mTexture, NULL, renderQuad)
    }
...
loadMedia()
{
	...
	; Load Foo' texture
	if (!gFooTexture.loadFromFile("BMP\foo.png"))
	{
		MsgBox SDL2.Log("Failed to load Foo' texture image!`n")
		success := false
	}
	
	; Load background texture
	if (!gBackgroundTexture.loadFromFile("BMP\background.png"))
	{
		MsgBox SDL2.Log("Failed to load background texture image!`n")
		success := false
	}
