...
loadMedia()
{
	...
	else
	{
		...
		{
			MsgBox SDL2.Log("Unable to render prompt texture!`n")
			success := false
		}
	}
    ...
    else
    {
        ...
        while (!quit)
        {
            ...
            while (SDL2.PollEvent(&e) != 0)
            {
                ...
                else if (e.type == SDL.SDL_KEYDOWN && e.key.keysym.sym == SDL.SDLK_RETURN)
                {
                    startTime := SDL2.GetTicks()
                }
            }

            ; Set text to be rendered
            timeText := ""
            timeText .= "Milliseconds since start time " (SDL2.GetTicks() - startTime)
