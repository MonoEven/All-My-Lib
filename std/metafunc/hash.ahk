class _hash
{
    ...
    static int2bin(n, count := 24)
    {
        tmp := []
        
        for y in _hash.range(0, count)
        {
            y := count - 1 - y
            tmp.push((n >> y) & 1)
        }
    ...
    fill_text()
    {
        for i in _hash.range(0, this.message.Length)
        {
            c := _hash.int2bin(ord(this.message[i + 1]), 8)
            this.ciphertext .= _hash.join(c)
        }
        ...
        if (length <= 255)
            length := _hash.join(_hash.int2bin(length, 8))
        else
        {
            length := _hash.int2bin(length, 16)
            temp := _hash.join(_hash.cut(length, 8, 12)) . _hash.join(_hash.cut(length, 12, 16)) . _hash.join(_hash.cut(length, 0, 4)) . _hash.join(_hash.cut(length, 4, 8))
            length := temp
        }
    ...
    FF(mj, s, ti)
    {
        ...
        this.A := mod((this.B + temp), _hash.pow(2, 32))
        this.change_pos()
    }
    ...
    group_processing(typed := "lower")
    {
        ...
        for i in _hash.range(0, 16)
        {
            ...
            ; 获取每一段的标准十六进制形式
            for j in _hash.range(0, 8)
            {
                j *= 4
                temp := _hash.cut(_hash.cut(ciphertext, i, i + 32), j, j + 4)
                temp := StrSplit(_hash.hex(_hash.int(_hash.join(temp), 2)), "")
                num .= temp[3]
            }
            ...
            for j in _hash.range(0, 4)
            {
                j := 8 - j * 2
                temp := _hash.join(_hash.cut(num, j - 2, j))
                num_tmp .= temp
            }

            num := ""
            
            for i in _hash.range(0, strlen(num_tmp))
                num .= _hash.join(_hash.int2bin(_hash.int(StrSplit(num_tmp, "")[i + 1], 16), 4))
            M.push(num)
        }
        ...
        this.A := mod((this.A + this.init_A), _hash.pow(2, 32))
        this.B := mod((this.B + this.init_B), _hash.pow(2, 32))
        this.C := mod((this.C + this.init_C), _hash.pow(2, 32))
        this.D := mod((this.D + this.init_D), _hash.pow(2, 32))
        
        answer := ""
        
        for register in [this.A, this.B, this.C, this.D]
        {
            register := StrSplit(SubStr(_hash.hex(register), 3), "")
            
            for i in _hash.range(0, 4)
            {
                i := 8 - i * 2
                answer .= _hash.join(_hash.cut(register, i - 2, i))
            }
        }
...
hash(obj)
{
    if obj is string || obj is number
    {
        ...
        return ret_hash.group_processing()
    }
    if hasmethod(obj, "__hash")
        return obj.__hash()
